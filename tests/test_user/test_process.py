...
from flask_site_framework.user.process import exc, email, password, totp
...
class ProcessEmailBeginTest(TestCase):
    ...
    @patch('flask_site_framework.user.process.email.url_for')
    @patch('flask_site_framework.user.process.email.db')
    def test_begin_email_confirmation__disabled__new(self, mock_db, mock_url_for):
        with mock_app({'USERS_VERIFY_EMAIL': False, 'USERS_VERIFY_EXPIRATION': 3600, 'SITE_NAME': 'Test Site'}) as app:
            user = MockUser()
            ...
            app.email.create.assert_called_once_with(subject, to=['test@test.test'])
            app.email.create().render.assert_called_once_with(
            ...
            app.email.create().render().send_message.assert_called_once()
    ...
    def test_begin_email_confirmation__new__same_email(self, mock_db, mock_url_for):
        with mock_app({'USERS_VERIFY_EMAIL': True, 'USERS_VERIFY_EXPIRATION': 3600, 'SITE_NAME': 'Test Site'}) as app:
            ...
            self.assertEqual(app.email.create.call_count, 2)
            self.assertEqual(app.email.create().render.call_count, 2)
            self.assertEqual(app.email.create().render().send_message.call_count, 2)
    ...
    def test_begin_email_confirmation__failed_to_send(self, mock_db, mock_url_for):
        with mock_app({'USERS_VERIFY_EMAIL': True, 'USERS_VERIFY_EXPIRATION': 3600, 'SITE_NAME': 'Test Site'}) as app:
            user = MockUser()
            app.email.create().render().send_message.side_effect = Exception("send fail")
...
class ProcessEmailCompleteTest(TestCase):
    @patch('flask_site_framework.user.process.email.db')
    @patch('flask_site_framework.user.process.email.User')
...
class ProcessPasswordBeginTest(TestCase):
    @patch('flask_site_framework.user.process.password.url_for')
    @patch('flask_site_framework.user.process.password.db')
...
class ProcessPasswordCheckTest(TestCase):
    @patch('flask_site_framework.user.process.password.url_for')
    @patch('flask_site_framework.user.process.password.db')
    @patch('flask_site_framework.user.process.password.User')
...
class ProcessTOTPBeginSetupTest(TestCase):
    @patch('flask_site_framework.user.process.totp.db')
...
class ProcessTOTPCompleteSetupTest(TestCase):
    @patch('flask_site_framework.user.process.totp.db')
    @patch('flask_site_framework.user.process.totp.validate_totp')
...
class ProcessTOTPGetTOTPURITest(TestCase):
    @patch('flask_site_framework.user.process.totp.get_totp')
...
class ProcessTOTPGetTOTPQRTest(TestCase):
    @patch('flask_site_framework.user.process.totp.get_totp_uri')
    @patch('qrcode.QRCode')
