...
from bc_fsf_user import events
from bc_fsf_user.permissions import (
    Permission,
    Role,
)
from bc_fsf_user.process import exc as bc_fsf_user_proc_exc
...
class TestCurrentUser(TestCase):
    ...
    @patch('bc_fsf_user.events.get_current_user')
    ...
    @patch('bc_fsf_user.events.get_current_user')
    @patch('bc_fsf_user.events.RoleGroup')
...
class TestAfterPermCheck__EmailConf(TestCase):
    ...
    @patch('bc_fsf_user.events.redirect')
    @patch('bc_fsf_user.events.url_for')
...
class TestCreateUser__Create(TestCase):
    ...
    def test_create__integrityerr(self, mock_db, mock_user_cls, mock_event):
        ...
        with patch('bc_fsf_user.events.IntegrityError', new=MockIntErr):
